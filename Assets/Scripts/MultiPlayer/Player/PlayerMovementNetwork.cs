...
{
	public class PlayerMovementNetwork : Photon.MonoBehaviour {
		...
		// For Photon Network
		Vector3 syncedPosition = Vector3.zero;
		Quaternion syncedRotation = Quaternion.identity;
		...
		void FixedUpdate ()
		{
			...
			{
				anim.SetBool ("IsWalking", ((transform.position - syncedPosition).magnitude > 0.05));
			}
		}
		...
		void Update ()
		{
			if (!photonView.isMine) 
			{
				...
				{
					playerRigidbody.position = syncedPosition;
					playerRigidbody.rotation = syncedRotation;
				}
			}
		}
